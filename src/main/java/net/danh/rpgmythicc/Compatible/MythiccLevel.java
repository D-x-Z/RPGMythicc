...
public class MythiccLevel extends DoubleStat implements ItemRestriction {
    ...
    public boolean canUse(@NotNull RPGPlayer rpgPlayer, @NotNull NBTItem nbtItem, boolean b) {
        ...
        if (Level.isLoad(rpgPlayer.getPlayer()) && Level.getLevel(rpgPlayer.getPlayer()) < level && !rpgPlayer.getPlayer().hasPermission("RPGMythicc.admin")) {
            if (b) {
                rpgPlayer.getPlayer().sendTitle(Files.colorize("&c&lCảnh Báo"), Files.colorize("&3Cấp độ của bạn không đủ để dùng vật phẩm này"), 10, 10, 10);
